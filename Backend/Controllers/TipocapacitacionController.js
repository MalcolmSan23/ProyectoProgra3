...
controller.list = (req, res) => {
  req.getConnection((error, conn) => {
    conn.query(
      `SELECT Tipocapacitacion.idtipocapa, Tipocapacitacion.tipocapa, Tipocapacitacion.estado
    FROM Tipocapacitacion`,
      (err, Tipocapacitacion) => {
        if (err) {
          res.json(err);
        }
...
controller.update = (req, res) => {
  ...
  req.getConnection((err, conn) => {
    conn.query(
      "update Tipocapacitacion set ? where idtipocapa =?",
      [nuevo_Tipocapacitacion, idtipocapa],
      (err, rows) => {
...
controller.delete = (req, res) => {
  const { idtipocapa } = req.params;
  req.getConnection((err, conn) => {
    conn.query("delete from Tipocapacitacion where idtipocapa =?", [idtipocapa], (err, rows) => {
