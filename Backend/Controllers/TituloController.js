...
controller.list = (req, res) => {
    req.getConnection((error,conn) =>{
        conn.query('select *from Titulo',(err,Titulo) =>{
            if(err){
                res.json(err);
            }
            res.json(Titulo);
...
controller.save = (req,res) =>{
    const data = req.body;
   req.getConnection((err,conn)=> {
       conn.query('insert into Titulo set?', [data], (err,idtitulo) => {
        res.json(Titulo);
       });
   })
};
...
controller.update = (req,res) =>{
    ...
    req.getConnection((err, conn) => {
        conn.query('update Titulo set ? where idtitulo=?', [nuevo_Titulo, idtitulo], (err,rows) =>{ 
